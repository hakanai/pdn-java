...
/**
 * Custom composite operation for Reflect.
 *
 * ???
 */
public class ReflectComposite extends AbstractComposite {
    public static final ReflectComposite INSTANCE = new ReflectComposite();

    private static final int[] masTable = {
    ...
    protected void mixPixels(int[] srcPixels, int[] dstInPixels, int[] dstOutPixels, int off) {
        ...
        // Blend logic here
        int y = srcA * invert(dstInA) + 0x80;
        y = ((y >> 8) + y) >> 8;
        int totalA = y + dstInA;

        int dstOutR = clampedDivide(dstInR * dstInR, invert(srcR));
        int dstOutG = clampedDivide(dstInG * dstInG, invert(srcG));
        int dstOutB = clampedDivide(dstInB * dstInB, invert(srcB));
        int dstOutA = clampedDivide(dstInA * dstInA, invert(srcA));
    ...
    private int doSomething(int src, int dstIn) {
        if (dstIn == 255) {
            return 255;
        } else {
            int i = invert(dstIn) * 3;
            int M = masTable[i];
            int A = masTable[i + 1];
            int S = masTable[i + 2];
            return clamp(src * src * M + A >> S);
        }
    }
}
